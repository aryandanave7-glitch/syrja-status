...
app.get("/get-invite/:id", async (req, res) => {
    ...
    try {
        ...
        if (item && item.code) {
            // --- NEW: Block Check ---
            // Check if the target user (item) has blocked the searcher
            if (item.blockedSearchers && item.blockedSearchers.includes(searcherPubKey)) {
                ...
                // Return 404 Not Found to make it seem like the ID doesn't exist to the blocked user
            ...
            if (item.privacy === 'private') {
                console.log(`🔒 Attempt to resolve private Syrja ID denied (not blocked, but private): ${fullId}`);
